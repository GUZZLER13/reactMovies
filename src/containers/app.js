...
const API_KEY = "2bc146c5cb9a81f1bb6ccb3e31bf9c53";
...
class App extends Component {
    ...
    applyVideoToCurrentMovie() {
        const request = axios
            .get(
                ...
                }?api_key=${API_KEY}&append_to_response=videos&include_adult=false`
            )
            .then(
                function(response) {
                    if (
    ...
    searchMovie(textSearch) {
        if (textSearch) {
            const request = axios
                .get(
                    `${API_END_POINT}search/movie?api_key=${API_KEY}&${DEFAULT_PARAM}&query=${textSearch}`
                )
                .then(
                    function(response) {
    ...
    setRecommendation() {
        const request = axios
            .get(
                `${API_END_POINT}movie/${
                    this.state.currentMovie.id
                }/recommendations?api_key=${API_KEY}&language=fr`
            )
            .then(
                function(response) {
    ...
    render() {
        const renderMovieList = () => {
            ...
            return (
                <MovieList
                    ...
                    callBackMovieClick={this.callBackMovieClick.bind(this)}
                />
            );
        };
        const renderMovie = () => {
            if (this.state.currentMovie.videoId)
                return (
                    <div>
                        <MovieVideo videoId={this.state.currentMovie.videoId} />
                        <MovieDetail
        ...
        return (
            <div>
                ...
                <div className="row">
                    <div className="col-md-8">{renderMovie()}</div>
